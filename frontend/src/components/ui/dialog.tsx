...
function DialogOverlay({ className, ...props }: DialogOverlayProps) {
  return (
    <DialogOverlayPrimitive
      className={cn('fixed inset-0 z-[140] bg-black/50', className)}
      {...props}
    />
  );
}
  ...
  return (
    <DialogPortalPrimitive>
      ...
      <DialogContentPrimitive
        className={cn(
          'bg-background fixed top-[50%] left-[50%] z-[150] grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg sm:max-w-lg',
          className,
        )}
        {...props}
      >
