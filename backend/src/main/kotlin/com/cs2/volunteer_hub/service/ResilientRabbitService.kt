...
class ResilientRabbitService(
        ...
        return CompletableFuture.supplyAsync(
                {
                    try {
                        logger.debug(
                                "Sending message to RabbitMQ - Exchange: $exchange, RoutingKey: $routingKey"
                        )
    ...
    /**
     * Fallback method when RabbitMQ circuit breaker is open Stores the message in database for
     * later retry
     */
    ...
    private fun sendMessageFallback(
        ...
        logger.warn(
                "RabbitMQ circuit breaker activated. Storing message to pending queue. Reason: ${throwable.message}"
        )
        logger.warn(
                "Message type: ${message::class.simpleName}, Exchange: $exchange, RoutingKey: $routingKey"
        )
    ...
    /**
     * Send message without circuit breaker (for internal/critical operations) Use this only for
     * operations that should not be circuit-broken
     */
    fun sendMessageDirect(exchange: String, routingKey: String, message: Any) {
        logger.debug(
                "Sending direct message (no circuit breaker) to RabbitMQ - Exchange: $exchange, RoutingKey: $routingKey"
        )
        rabbitTemplate.convertAndSend(exchange, routingKey, message)
    }
}
