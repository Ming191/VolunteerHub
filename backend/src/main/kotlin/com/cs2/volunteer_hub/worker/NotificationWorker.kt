...
class NotificationWorker(
        ...
        fun handleNotification(message: NotificationMessage) {
                try {
                        logger.info(
                                "Processing notification for user ${message.userId}: ${message.title}"
                        )
                        ...
                        logger.info(
                                "Successfully processed notification for user ${message.userId}"
                        )
                } catch (e: Exception) {
                        logger.error(
        ...
        fun handleRegistrationStatusUpdate(message: RegistrationStatusUpdateMessage) {
                try {
                        ...
                        val registration =
                                registrationRepository.findById(message.registrationId).orElse(null)
                        if (registration == null) {
                                logger.warn(
                                        "Registration not found: ${message.registrationId}. Skipping."
                                )
                                return
                        }
                        ...
                        logger.info(
                                "Generated notification body for status ${message.status}: $body"
                        )
                        ...
                        if (body != null) {
                                ...
                                logger.info(
                                        "Saved notification to database for user ${userToNotify.id}"
                                )

                                // Check settings
                                val userSettings =
                                        userSettingsRepository.findByUserId(userToNotify.id)
                                logger.info(
                                        "User settings for ${userToNotify.id}: pushEnabled=${userSettings?.pushNotificationsEnabled}"
                                )
                                ...
                                if (userSettings?.pushNotificationsEnabled == true) {
                                        ...
                                        logger.info(
                                                "Sent FCM push notification call for user ${userToNotify.id}"
                                        )
                                } else {
                                        logger.info(
                                                "Skipping push notification for registration update to user ${userToNotify.id} (disabled or settings null)"
                                        )
                                }
                                ...
                                logger.warn(
                                        "Notification body is null for status ${message.status}"
                                )
                        }
                } catch (e: Exception) {
                        logger.error(
