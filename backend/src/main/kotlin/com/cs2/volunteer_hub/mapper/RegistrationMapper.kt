...
class RegistrationMapper {
    ...
    /**
     * Map Registration to RegistrationResultResponse
     * Used when user registers for an event
     */
    fun toRegistrationResultResponse(registration: Registration): RegistrationResultResponse {
        val message = when (registration.status) {
            RegistrationStatus.WAITLISTED -> "Event is full. You've been added to the waitlist."
            RegistrationStatus.PENDING -> "Registration successful. Waiting for organizer approval."
            RegistrationStatus.APPROVED -> "Registration approved! You're confirmed for this event."
    ...
    /**
     * Map optional Registration to RegistrationStatusResponse
     * Used for checking if user is registered for an event
     */
